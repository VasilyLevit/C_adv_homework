...
struct snake_t initSnake(int x, int y, size_t tsize)
{
	...
	// создаем количество элеметов по длине змеи 
...
{
	for (int i = snake.tsize - 1; i > 0; i--)  //идем по хвосту
		snake.tail[i] = snake.tail[i-1]; // двигаем хвост присваивая каждому элементу координаты левого элемента
	...
	if (snake.x < 0) //контролируем, чтобы не появилиь отрицательные индексы
